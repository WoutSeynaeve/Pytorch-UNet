...
def calculateLogicLoss(output_tensor,weaklabels,configuration,batch_n,printLosses = False):
    ...
    #Domain-directional
    domDirec = configuration.get("Domain-directional")
    if domDirec[0]:
        addloss = 0
        addloss += ifXthenYatRelation(output_tensor, 1, 0, "left")*domDirec[1]
        addloss += ifXthenYatRelation(output_tensor, 1, 0, "right")*domDirec[1]
        addloss += ifXthenYatRelation(output_tensor, 1, 0, "under")*domDirec[1]
        addloss += ifXthenYatRelation(output_tensor, 1, 0, "above")*domDirec[1]

        addloss += ifXthenYatRelation(output_tensor, 2, 0, "left")*domDirec[1]
        addloss += ifXthenYatRelation(output_tensor, 2, 0, "right")*domDirec[1]
        addloss += ifXthenYatRelation(output_tensor, 2, 0, "under")*domDirec[1]
        addloss += ifXthenYatRelation(output_tensor, 2, 0, "above")*domDirec[1]

        addloss += ifXthenYatRelation(output_tensor, 3, 0, "left")*domDirec[1]
        addloss += ifXthenYatRelation(output_tensor, 3, 0, "right")*domDirec[1]
        addloss += ifXthenYatRelation(output_tensor, 3, 0, "under")*domDirec[1]
    ...
    #Mean, max, min for cube, sphere, cylinder
    #     2.675564853556486 9.18 0.51
    # 1.7040585774058568 4.63 0.37
    # 2.5163807531380793 8.07 0.5
    #minimum size global constraint: atleast 0.35% of the image is filled by each shape
    minSizeShapes = configuration.get("MinSizeShapes")
    if minSizeShapes[0]:
        if minSizeShapes[2]:
            addloss = atleast_p_percent_is_class(output_tensor,[1],2.51/100) #cylinder
            addloss += atleast_p_percent_is_class(output_tensor,[2],1.7/100) #sphere
            addloss += atleast_p_percent_is_class(output_tensor,[3],2.6/100) #cube
            loss += addloss*minSizeShapes[1]
        else:
            addloss = atleast_p_percent_is_class(output_tensor,[1],0.51/100) #cylinder
            addloss += atleast_p_percent_is_class(output_tensor,[2],0.37/100) #sphere
            addloss += atleast_p_percent_is_class(output_tensor,[3],0.5/100) #cube
            loss += addloss*minSizeShapes[1]
...
def calculateLogicLossHeterogeneous(output_tensor,weaklabels,configuration,batch_n,printLosses = False):
    ...
    #20% percentage 20% region 20% directional 10% adjacency 10% Point 10% Scribble 10% Bbox
    weaklabel_todo = ""
    if configuration.get("heteroConfig") == 0:
        if abs(batch_n) >= 1 and abs(batch_n) < 24:
            weaklabel_todo = "to_do_adjacency"
        elif abs(batch_n) >= 24 and abs(batch_n) < 48:
            weaklabel_todo = "to_do_point"
        elif abs(batch_n) >= 48 and abs(batch_n) < 72:
            weaklabel_todo = "to_do_scribble"
        elif abs(batch_n) >= 72 and abs(batch_n) < 96:
            weaklabel_todo = "to_do_bounding_box"
        elif abs(batch_n) >= 96 and abs(batch_n) < 144:
            weaklabel_todo = "to_do_directional"
        elif abs(batch_n) >= 144 and abs(batch_n) < 192:
            weaklabel_todo = "to_do_region"
        elif abs(batch_n) >= 192 and abs(batch_n) < 240:
            weaklabel_todo = "to_do_imagelevel"
        else:
            print("should not be here123")
            
    elif configuration.get("heteroConfig") == 1:
        if abs(batch_n) >= 1 and abs(batch_n) < 24:
            weaklabel_todo = "to_do_adjacency"
        elif abs(batch_n) >= 24 and abs(batch_n) < 48:
            weaklabel_todo = "to_do_imagelevel"
            ...
            print("should not be here123")
    elif configuration.get("heteroConfig") == 2:
        if abs(batch_n) >= 0 and abs(batch_n) < 34:
            weaklabel_todo = "to_do_adjacency"
        elif abs(batch_n) >= 34 and abs(batch_n) < 68:
            weaklabel_todo = "to_do_point"
        elif abs(batch_n) >= 68 and abs(batch_n) < 102:
            weaklabel_todo = "to_do_scribble"
        elif abs(batch_n) >= 102 and abs(batch_n) < 136:
            weaklabel_todo = "to_do_bounding_box"
        elif abs(batch_n) >= 136 and abs(batch_n) < 170:
            weaklabel_todo = "to_do_directional"
        elif abs(batch_n) >= 170 and abs(batch_n) < 204:
            weaklabel_todo = "to_do_region"
        elif abs(batch_n) >= 204 and abs(batch_n) < 240:
            weaklabel_todo = "to_do_imagelevel"
        else:
            print("should not be here123")
    ...
    imglvl = configuration.get("ImageLevel")
    imlvlpercdict = {'cylinder': image_level_label[1][1][0][:-1],'sphere': image_level_label[2][1][0][:-1],'cube': image_level_label[3][1][0][:-1]}
    if weaklabel_todo == "to_do_imagelevel":
        for label in image_level_label:
            shape,percentage = label[0][0],label[1][0]
            if shape != 'background':
                if imglvl[1]:
    ...
    #Bounding Boxes
    bboxes = configuration.get("BBox")
    if weaklabel_todo == "to_do_bounding_box":
        for bbox in bounding_box:
            shape,x1,x2,y1,y2,percentage = bbox
            shape,x1,x2,y1,y2,percentage = shape[0],x1[0],x2[0],y1[0],y2[0],percentage[0]
            if True:
    ...
    if minSizeShapes[0]:
        if minSizeShapes[2]:
            ...
            for classes in range(1,4):
                addloss = atleast_p_percent_is_class(output_tensor,[classes],0.37/100)
                if printLosses:
    ...
    #scribble
    scribbles = configuration.get('Scribbles')
    if weaklabel_todo == "to_do_scribble":
        for scribb in scribble:          
            first_entry = scribb[0][0].split(",")  # Split the first string
            shape = first_entry[1]  # Extract the shape name
            coords = [[int(first_entry[2]), int(first_entry[3])]]  # First coordinate
    ...
    #area
    areainfo = configuration.get('Area')
    if weaklabel_todo == "to_do_region":
        for arealbl in area:
            ar1,ar2 = arealbl[1][0].split(";")
            shape = arealbl[0][0]
            addloss = 0
    ...
    if weaklabel_todo == "to_do_directional":
        for rel in relation:
            addloss = 0
            shape1,relat,shape2 = rel[0][0],rel[1][0],rel[2][0]
            if relations[4]:
    ...
    #adjacency
    adjacencies = configuration.get("Adjacency")
    if weaklabel_todo == "to_do_adjacency":
        presentAdjacencies = set()
        if len(adjacency) > 0:
            for adj in adjacency:
                shape1, shape2 = adj[0][0],adj[1][0]
