...
class_values = {
    "background": 0,
    "Person": 2, #dit is omgekeerd omdat in de annotations het ook omgekeerd dus fout staat
    "Horse": 1
}
...
def parse_dataCOCO(data):
    ...
    NOBOUNDINGBOX = False
    for line in data:
        if line.startswith("NOBOUNDINGBOX"):
            NOBOUNDINGBOX = True
    ...
    return image_level_label, bounding_box, point, relation, soft_relation, scribble, area, full_bounding_box, adjacency, NOBOUNDINGBOX


def calculateLogicLoss(output_tensor,weaklabels,configuration,batch_n = -1000,printLosses = False):
    ...
    image_level_label, bounding_box, point, relation, soft_relation, scribble, area, full_bounding_box, adjacency,NoBoundingBox = weaklabels[0]
    loss = torch.zeros(1, device="cuda") 

    #Image-level
    imglvl = configuration.get("ImageLevel")
    imlvlpercdict = {'Person': image_level_label[1][1][0][:-1],'Horse': image_level_label[2][1][0][:-1]}
    if imglvl[0]:
        ...
        for bbox in bounding_box:
                    ...
                    if bboxes[4]:
                        ...
                        if not bboxes[5]:
                            if bboxes[0][1]:
                                ...
                                addloss = atleast_p_percent_is_class_in_bounding_box(output_tensor,[class_values[shape]],0.2,int(x1),int(x2),int(y1),int(y2))
                                loss += addloss*bboxes[0][2]
                    else:
                        if bboxes[0][1]:
                            addloss = about_p_percent_is_class_in_bounding_box(output_tensor,[class_values[shape]],float(percentage[:-1])/100,int(x1),int(x2),int(y1),int(y2))
            ...
            if bboxes[1][0]:
                ...
                if bboxes[3] == "linear":
                    ...
                    if int(y2) < H:
                        addloss += about_p_percent_is_class_in_bounding_box(output_tensor,[class_values[shape]],0,int(x1),int(x2),int(y2),H)
                    if int(x2) < W:
                        addloss += about_p_percent_is_class_in_bounding_box(output_tensor,[class_values[shape]],0,int(x2),W,0,H)
                elif bboxes[3] == "prob":
                    if int(x1) > 1:
    ...
    if points[0]:
        for scribb in scribble:          
            ...
            # Convert the remaining coordinates to integers
            coords += [list(map(int, item[0].split(','))) for item in scribb[1:]]
            addloss = scribble_loss(output_tensor,coords[:1],class_values[shape],"all")

            if printLosses:
                print("Loss for scribble for shape",shape," = ",addloss*points[1])
...
def calculateLogicLossHeterogeneous(output_tensor,weaklabels,configuration,batch_n,printLosses = False):
    ...
    if abs(batch_n) <= 130 and abs(batch_n) > 104: #20% percentages
        #Image-level
        imglvl = configuration.get("ImageLevel")
        imlvlpercdict = {'Person': image_level_label[1][1][0][:-1],'Horse': image_level_label[2][1][0][:-1]}
        if imglvl[0]:
    ...
    elif abs(batch_n) <= 78 and abs(batch_n) > 39: #30% bbox
        #Bounding Boxes
        bboxes = configuration.get("BBox")
        if bboxes[0][0]:
    ...
    elif abs(batch_n) <= 39 and abs(batch_n) > 0: #30% bbox
        #scribble
        scribbles = configuration.get('Scribbles')
        if scribbles[0]:
            for scribb in scribble:          
    ...
    elif abs(batch_n) <= 104 and abs(batch_n) > 78: #20% point
        points = configuration.get('Point')
        if points[0]:
            for scribb in scribble:          
                first_entry = scribb[0][0].split(",")  # Split the first string
