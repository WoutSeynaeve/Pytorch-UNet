...
from LogicLossVOC.WeakLabelLogicLossCLEVR import calculateLogicLoss,calculateLogicLossHeterogeneous
...
def train_model(
    ...
    experimentFileName = f"./experimentResultsCLEVRheterog/experiment_{configuration}.txt"
    writeInfo = ""
    for k in configuration_dict.keys():
        if k == 'BBox':
            ...
            writeInfo += "heterogeneous configuration: " + str(configuration_dict[k]) + "\n"
        ...
        for epoch in range(1, epochs + 1):
            ...
            for batch in train_loader:
                ...
                with torch.autocast(device.type if device.type != 'mps' else 'cpu', enabled=amp):
                    ...
                    loss = 0
                    if True: #batchid goes from 1 to 239 (batchid*-1 is the mirrored version of batchid)
                        if logiclossMult > 0:
                            loss += logiclossMult*calculateLogicLossHeterogeneous(masks_pred,weaklabel,configuration_dict,batch_id)[0]
                    else:
                        loss += cross_entropy(masks_pred,mask,H,W,device)
                        loss += diceLoss(masks_pred,mask,H,W,device)
